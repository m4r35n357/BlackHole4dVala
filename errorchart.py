...
#		eThI = InterpolatedUnivariateSpline(tau, eTh, k = 1)
        	eVI = InterpolatedUnivariateSpline(tau, ev, k = 1)
		eEI = InterpolatedUnivariateSpline(tau, ee, k = 1)
        	eLI = InterpolatedUnivariateSpline(tau, el, k = 1)
		eQI = InterpolatedUnivariateSpline(tau, eq, k = 1)
	...
	for i in range(len(tauI)):
		...
		ax2.plot(tauI[i], eVI(tauI[i]), color='blue', linestyle='-', marker=',', markersize=1, zorder=9)
		ax2.plot(tauI[i], eEI(tauI[i]), color='red', linestyle='-', marker=',', markersize=1, zorder=8)
		ax2.plot(tauI[i], eLI(tauI[i]), color='magenta', linestyle='-', marker=',', markersize=1, zorder=9)
		ax2.plot(tauI[i], eQI(tauI[i]), color='cyan', linestyle='-', marker=',', markersize=1, zorder=8)
